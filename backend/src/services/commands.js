import { normalizeSubject } from "../utils/subjects.js";
import {
...
import { loadStore, persistStore, resetUserData } from "./store.js";
...
function findTask(store, name) {
  ...
  return store.tasks.find(
    (t) => t.title.toLowerCase() === lower || t.title.toLowerCase().includes(lower)
  );
}
...
function parseCommand(raw, store) {
  ...
  if (/^reset$/.test(lower)) return { type: "reset" };
  if (/^export$/.test(lower)) return { type: "export" };

  const addSubject = input.match(/^add subject:\s*(.+)$/i);
  if (addSubject) {
    const subject = ensureSubject(store, addSubject[1]);
    return { type: "mutate", message: `Subject "${subject}" add ho gaya.`, level: "success" };
  }
  ...
  if (setGoal) {
    ...
    return { type: "mutate", message: `Goal set: ${subject} → ${setGoal[2]}h`, level: "success" };
  }
  ...
  if (addTask) {
    ...
    return { type: "mutate", message: `✓ Task added: "${rest}" [${priority}]`, level: "success" };
  }

  const doneTask = input.match(/^done\s+(.+)$/i);
  if (doneTask) {
    const task = findTask(store, doneTask[1]);
    if (!task) return { type: "message", text: `Task "${doneTask[1]}" nahi mila.`, level: "error" };
    task.status = "completed";
    task.completedAt = new Date().toISOString();
    return { type: "mutate", message: `✓ Done: "${task.title}"`, level: "success" };
  }

  const deleteTask = input.match(/^delete task:\s*(.+)$/i);
  if (deleteTask) {
    const task = findTask(store, deleteTask[1]);
    if (!task) return { type: "message", text: `Task "${deleteTask[1]}" nahi mila.`, level: "error" };
    store.tasks = store.tasks.filter((t) => t.id !== task.id);
    return { type: "mutate", message: `Task "${task.title}" delete ho gaya.`, level: "success" };
  }

  const deleteSession = input.match(/^delete session\s+(\d+)$/i);
  if (deleteSession) {
    const session = findSession(store, deleteSession[1]);
    if (!session) return { type: "message", text: `Session #${deleteSession[1]} nahi mila.`, level: "error" };
  ...
  if (archiveSession) {
    ...
    if (!session) return { type: "message", text: `Session #${archiveSession[1]} nahi mila.`, level: "error" };
    session.archived = true;
    return { type: "mutate", message: `Session #${session.id} archived.`, level: "success" };
  }

  const editSession = input.match(/^edit session\s+(\d+)\s+(.+)$/i);
  if (editSession) {
    const session = findSession(store, editSession[1]);
    if (!session) return { type: "message", text: `Session #${editSession[1]} nahi mila.`, level: "error" };
    ...
    if (notesMatch) session.notes = notesMatch[1].trim();
    return { type: "mutate", message: `Session #${session.id} updated.`, level: "success" };
  }
...
export function executeCommand(userId, raw) {
  ...
  if (result.type === "reset") {
    ...
    return {
      type: "message",
      message: "Sab clear ho gaya. Fresh start! 🔄",
      level: "success",
      mutated: true,
    };
  }
  if (result.type === "export") {
    return { type: "export", store: loadStore(userId), level: "success" };
  }
  if (result.type === "message") {
    return { ...result, message: result.text, mutated: false };
  }
