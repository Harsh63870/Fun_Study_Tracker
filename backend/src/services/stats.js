...
export function getBurndownData(store) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const pending = store.tasks.filter(
      (t) => t.status === "pending" && new Date(t.createdAt) <= endOfDay
    ).length;
...
export function buildStatsPayload(store) {
  const subjects = getSubjectHours(store);
  return {
    totalHours: getTotalHours(store),
    todayHours: getTodayHours(store),
    averagePerDay: getAverageHoursPerDay(store),
    streak: getStudyStreak(store),
    progress: getProgressPercentage(store),
    tasks: getTaskCompletionRate(store),
    mostStudied: getMostStudiedSubject(store),
    subjectBreakdown: Object.entries(subjects)
      .sort((a, b) => b[1] - a[1])
      .map(([subject, hours]) => ({ subject, hours })),
    weekly: getWeeklyHours(store),
    burndown: getBurndownData(store),
    recommendation: getRecommendation(store),
  };
}
